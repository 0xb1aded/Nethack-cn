...
#define mk_corpse(dest) (((dest & XKILL_NOCORPSE) != 0) ? AD_DGST : AD_PHYS)
        ...
        if (!DEADMONSTER(etmp->emon)) {
            ...
            if (DEADMONSTER(etmp->emon)) {
                if (seeit)
                    pline("不幸的是, %s仍然被压碎了.", /*修改语序:pline("不幸的是, 对于%s, %s仍然被压碎着.",*/
                          mon_nam(etmp->emon)); /*修改语序:mon_nam(etmp->emon), mhe(etmp->emon));*/
            } else {
    ...
    if (is_db_wall(etmp->ex, etmp->ey)) {
        ...
        if (is_pool(etmp->ex, etmp->ey) || is_lava(etmp->ex, etmp->ey))
            if (e_inview && !is_u(etmp)) {
                ...
                if (Hallucination)
                    pline("%s了一口%s然后消失了.",
                          E_phrase(etmp, "喝"), lava ? "熔岩" : "护城河里的水");
                else
                    pline("%s进了%s.", E_phrase(etmp, "掉"),
                          lava ? hliquid("熔岩") : "护城河");
            }
        svk.killer.format = NO_KILLER_PREFIX;
        Strcpy(svk.killer.name, "从吊桥上掉落");
