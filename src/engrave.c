    ...
    if (ep && ep->engr_txt[actual_text][0]) {
        ...
        case BURN:
            if (!Blind || can_reach_floor(TRUE)) {
                sensed = 1;
                pline("有字%s在了%s上.",
                      is_ice(x, y) ? "融化" : "烧", eloc);
            }
            break;
        case MARK:
            if (!Blind) {
                sensed = 1;
                pline("有一些涂鸦在%s上.", eloc);
            }
            break;
        case ENGR_BLOOD:
        ...
        if (sensed) {
            ...
            You("%s: \"%s\"%s", (Blind) ? "摸到上面的字" : "读到", et,
                endpunct);
            Strcpy(ep->engr_txt[remembered_text], ep->engr_txt[actual_text]); //debugfuzzer有问题:2
            ep->eread = 1;
            ep->erevealed = 1;
...
u_can_engrave(void)
{
    ...
    if (u.uswallow) {
        ...
        You_cant("写在%s上!", surface(u.ux, u.uy));
        return FALSE;
...
doengrave_sfx_item_WAN(struct _doengrave_ctx *de)
{
    ...
    case WAN_SLOW_MONSTER:
        if (!Blind) {
            Sprintf(de->post_engr_text, "%s上的臭虫移动变慢了!",
                    surface(u.ux, u.uy));
        }
        break;
    case WAN_SPEED_MONSTER:
        if (!Blind) {
            Sprintf(de->post_engr_text, "%s上的臭虫移动变快了!",
                    surface(u.ux, u.uy));
        }
        break;
    case WAN_POLYMORPH:
...
doengrave(void)
{
    ...
    de->otmp = getobj("用什么来写", stylus_ok, GETOBJ_PROMPT);
    ...
    if (de->len != 1 || (!strchr(de->ebuf, 'x') && !strchr(de->ebuf, 'X')))
        if (!u.uconduct.literate++)
            livelog_printf(LL_CONDUCT, "因写下\"%s\"而脱离文盲",
                           de->ebuf);
...
engrave(void)
{
    ...
    default:
        ...
        break;
    case DUST:
        finishverb = is_ice(u.ux, u.uy) ? "写在霜上"
                     : "写在灰尘上";
        break;
    case HEADSTONE:
    case ENGRAVE:
        ...
        break;
    case BURN:
        finishverb = is_ice(u.ux, u.uy) ? "把信息融化到冰上"
                     : "把信息烧到地板上";
        break;
    case MARK:
        finishverb = "玷污地牢";
