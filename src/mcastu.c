...
mcast_summon_mons(struct monst *mtmp)
{
    ...
    if (!count) {
        ...
        if (Invis && !perceives(mtmp->data)
            && (mtmp->mux != u.ux || mtmp->muy != u.uy))
            pline("%s出现在你附近%s!", mappear,
                  one ? "某处" : "各处");
        else if (Displaced && (mtmp->mux != u.ux || mtmp->muy != u.uy))
            pline("%s出现在你的幻影%s!", mappear,
                  one ? "旁" : "周围");
        else
            pline("%s从各处冒了出来!", mappear);
    }
}
...
mcast_weaken_you(struct monst *mtmp, int dmg)
{
    if (Antimagic) {
        ...
        if (Half_spell_damage)
            dmg = (dmg + 1) / 2;
        losestr(rnd(dmg),
                death_inflicted_by(kbuf, "力量丢失", mtmp),
                KILLED_BY);
        svk.killer.name[0] = '\0'; /* not killed if we get here... */
        monstunseesu(M_SEEN_MAGR);
    }
...
mcast_disappear(struct monst *mtmp)
{
    if (!mtmp->minvis && !mtmp->invis_blkd) {
        if (canseemon(mtmp))
            pline_mon(mtmp, "%s突然%s了!", Monnam(mtmp),
                      !See_invisible ? "消失" : "变透明");
...
mcast_lightning(struct monst *mtmp, int dmg)
{
    ...
    pline("一道闪电从你上面打下来!");
    reflects = ureflects("它从你的%s%s上弹开了.", "");
    orig_dmg = dmg = d(8, 6);
...
mcast_psi_bolt(int dmg)
{
    ...
    if (dmg <= 5)
        ...
        Your("%s突然非常痛!", body_part(HEAD));
    else
        Your("%s突然痛得要命!", body_part(HEAD));
    return dmg;
}
...
mcast_confuse_you(struct monst *mtmp)
{
    if (Antimagic) {
        ...
        if (Hallucination)
            You_feel("%s!", oldprop ? "更迷幻了" : "很迷幻");
        else
            You_feel("%s!", oldprop ? "更混乱了" : "很混乱");
        monstunseesu(M_SEEN_MAGR);
    }
}
...
buzzmu(struct monst *mtmp, struct attack *mattk)
{
    ...
    if (lined_up(mtmp) && rn2(3)) {
        nomul(0);
        if (canseemon(mtmp))
            pline_mon(mtmp, "%s对你挥舞%s!", Monnam(mtmp),
