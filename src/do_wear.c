...
Helmet_on(void)
{
    ...
    case DUNCE_CAP:
        if (uarmh && !uarmh->cursed) {
            if (Blind)
                pline("%s 了一会儿。", Tobjnam(uarmh, "震动"));
            else
                pline("%s %s一瞬。", Tobjnam(uarmh, "发光"),
                      hcolor(NH_BLACK));
            curse(uarmh);
    ...
    boolean voluntary) /* True: taking protective armor off on purpose */
{
    ...
    if (touch_petrifies(&mons[obj->corpsenm]) && !Stone_resistance) {
        ...
        if (how)
            Sprintf(hbuf, "%s %s", voluntary ? "移除" : "失去",
...
canwearobj(struct obj *otmp, long *mask, boolean noisy)
{
    ...
    if (is_helmet(otmp)) {
        ...
        if (uarmc) {
            if (noisy)
                You("无法在 %s 上穿戴护甲。", cloak_simple_name(uarmc));
            err++;
        } else if (uarm) {
            if (noisy)
    ...
    if (armor) {
        ...
        if (ring) {
            ...
            if (!has_head(gy.youmonst.data)) {
                You("没有头可以穿戴 %s。", ansimpleoname(obj));
                return ECMD_OK;
            }
...
*/
    ...
    if (otmp && otmp->otyp != AKLYS && !welded(otmp)) {
        ...
        pline("%s %s%s %s%s 从你的 %s%s。",
              !strncmp(thiswep, "尸体", 6) ? "这" : "你的",
              otherwep ? "另一" : "", thiswep, xfl ? "也" : "",
              otense(otmp, "滑落"), which, hand);
        /* xfl++; */
        otmp->quan = savequan;
        setuwep((struct obj *) 0);
...
select_off(struct obj *otmp)
{
    ...
    *buf = '\0'; /* lint suppression */
    ...
    if (otmp == uarm || otmp == uarmu) {
        ...
        if (why) {
            You("你不能%s脱下%s。", buf, the(xname(otmp)));
            set_bknown(why, 1);
            return 0;
        }
    }
...
do_takeoff(void)
{
    ...
    if (doff->what == W_WEP) {
        ...
        setuswapwep((struct obj *) 0);
        You("%s不再%s。", was_twoweap ? "已经" : "",
            was_twoweap ? "同时使用两种武器"
                        : "准备好第二把武器");
