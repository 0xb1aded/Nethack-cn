    ...
    if (mon) {
        ...
        case OBJ_MINVENT:
            if (cansee_hatchspot) {
                ...
                You_see("%s %s 从 %s！", monnambuf,
                        locomotion(mon->data, "掉出"), carriedby);
            }
            break;
#if 0
        case OBJ_MIGRATING:
    ...
    if (otmp && on_foot) { /* trip over something in particular */
        ...
        */
        ...
        if (!uarmf && otmp->otyp == CORPSE
            ...
            Sprintf(svk.killer.name, "被%s尸体绊倒",
...
see_lamp_flicker(struct obj *obj, const char *tailer)
{
    ...
    case OBJ_MINVENT:
        ...
        break;
    case OBJ_FLOOR:
        You_see("%s 闪烁%s.", an(xname(obj)), tailer);
        break;
    }
}
...
wiz_timeout_queue(void)
{
    ...
    if (u.uswldtim) {
        ...
        Sprintf(buf, "吞噬倒计时为%u.", u.uswldtim);
        putstr(win, 0, buf);
    }
    if (u.uinvault) {
        putstr(win, 0, "");
        Sprintf(buf, "地窖计数器为 %d.", u.uinvault);
        putstr(win, 0, buf);
    }
...
#ifdef VERBOSE_TIMER
        Sprintf(idbuf, "%s计时器", timeout_funcs[func_index].name);
#else
