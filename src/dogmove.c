        ...
        boolean devour)
{
    ...
    if (is_pool(mtmp->mx, mtmp->my) && !Underwater) {
        ...
        if (sawpet || (seeobj && canspotmon(mtmp))) {
            ...
            if (tunnels(mtmp->data))
                pline_mon(mtmp, "%s往里挖.", noit_Monnam(mtmp));
            else
                pline_mon(mtmp, "%s%s%s.",
                          devour ? noit_or_your_Monnam(mtmp)
                                 : noit_Monnam(mtmp),
                          devour ? "吞下了" : "吃下了", obj_name);
        } else if (seeobj) {
            obj_name = distant_name(obj, doname);
            pline("它%s%s.", devour ? "吞下了" : "吃下了", obj_name);
        }
    }
    if (obj->unpaid) {
    ...
    if (mtmp->data == &mons[PM_RUST_MONSTER] && obj->oerodeproof) {
        ...
        if (obj->unpaid) {
            ...
            pline("%s你需支付%ld %s.", objnambuf, oprice,
                  currency(oprice));
    ...
    if (svm.moves > edog->hungrytime + DOG_WEAK) {
        if (!carnivorous(mtmp->data) && !herbivorous(mtmp->data)) {
            ...
            if (cansee(mtmp->mx, mtmp->my))
                pline_mon(mtmp, "%s饿得神志不清.", Monnam(mtmp));
            else if (couldsee(mtmp->mx, mtmp->my))
                beg(mtmp);
            else
                You_feel("很担心%s.", y_monnam(mtmp));
            stop_occupation();
 ...
 newdogpos:
    if (nix != omx || niy != omy) {
        ...
        if (cursemsg[chi] && (wasseen || canseemon(mtmp))) {
            ...
            pline_mon(mtmp, "%s不情愿地%s%s%s.", noit_Monnam(mtmp),
                  vtense((char *) 0, locomotion(mtmp->data, "走")),
                  (is_flyer(mtmp->data) || is_floater(mtmp->data)) ? "过"
                                                                   : "上",
                  what);
        }
