...
really_kick_object(coordxy x, coordxy y)
{
    ...
    if (slide && !Blind)
        pline("啊! %s从%s上%s过.", Doname2(gk.kickedobj), surface(x, y),
              otense(gk.kickedobj, "滑"));
    ...
    xint16 dlev)          /* if !0 send to dlev near player */
{
    ...
    if (costly && shkp && price) {
        if (ESHK(shkp)->robbed > robbed) {
            You("拿走了价值 %ld %s 的物品!", price, currency(price));
...
otransit_msg(struct obj *otmp, boolean nodrop, boolean chainthere, long num)
{
    ...
    if (num || chainthere) {
        ...
        if (nodrop)
            ...
            Sprintf(eos(xbuf), "然后%s%s.",
                    otense(otmp, "掉落"), gg.gate_str);
        pline("%s%s", obuf, xbuf);
    } else if (!nodrop)
        pline("%s%s%s.", obuf, otense(otmp, "掉落"), gg.gate_str);
}
...
down_gate(coordxy x, coordxy y)
{
    ...
    if ((ttmp = t_at(x, y)) != 0 && ttmp->tseen && is_hole(ttmp->ttyp)) {
        gg.gate_str = (ttmp->ttyp == TRAPDOOR) ? "穿过活板门"
                                              : "穿过洞口";
        return MIGR_RANDOM;
    }
    return MIGR_NOWHERE;
