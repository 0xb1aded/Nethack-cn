...
doread(void)
{
    ...
    if (otyp == FORTUNE_COOKIE) {
        ...
        if (otyp == HAWAIIAN_SHIRT) {
            pline("%s带有%s。", flags.verbose ? "该设计图案" : "它",
    ...
    if (otyp != SCR_BLANK_PAPER) {
        ...
        if (Blind)
            ...
            pline(nodisappear ? "你阅读了卷轴。"
                              : "当你阅读卷轴时，它消失了。");
        if (confused) {
            if (Hallucination)
                pline("如此迷幻，你搞砸了...");
            else
                pline("困惑中，你%s了那些魔法咒语...",
                      silently ? "误解" : "念错");
        }
    }
    if (!seffects(scroll)) {
...
{
    pline("%s%s%s 片刻。", Yobjnam2(otmp, Blind ? "振动" : "发光"),
...
seffect_enchant_armor(struct obj **sobjp)
{
    ...
    if (confused) {
        ...
        if (Blind) {
            ...
            otmp->rknown = TRUE;
            pline("%s被一层%s %s %s覆盖！", Yobjnam2(otmp, "是"),
                  scursed ? "斑驳的" : "闪烁的",
    ...
    if (s > (special_armor ? 5 : 3) && rn2(s)) {
        otmp->in_use = TRUE;
        pline("%s猛烈地%s%s%s了一会儿，然后%s。", Yname2(otmp),
              otense(otmp, Blind ? "震动" : "发光"),
              (!Blind && !same_color) ? " " : "",
    ...
    pline("%s %s%s%s%s持续了%s。", Yname2(otmp),
          (s == 0) ? "猛烈地 " : "",
          otense(otmp, Blind ? "震动" : "发光"),
          (!Blind && !same_color) ? " " : "",
          (Blind || same_color)
...
seffect_confuse_monster(struct obj **sobjp)
{
    ...
    if (gy.youmonst.data->mlet != S_HUMAN || scursed) {
        ...
        if (!sblessed) {
            if (altfeedback)
                Your("%s 发麻%s.", hands, u.umconf ? "更厉害了" : "");
            else if (!u.umconf)
                Your("你的%s开始发出%s光。", hands, hcolor(NH_RED));
            else
                pline_The("你的%s的%s光芒增强了。", hcolor(NH_RED),
                          hands);
            incr += rnd(2);
        } else {
            if (altfeedback)
...
seffect_scare_monster(struct obj **sobjp)
{
    ...
    if (otyp == SCR_SCARE_MONSTER || !ct) {
        ...
        You_hear("%s %s.", (confused || scursed) ? "悲伤的哀号"
                 : "疯狂的笑声",
                 !ct ? "在远处" : "在近处");
    }
}
...
seffect_fire(struct obj **sobjp)
{
    ...
    *sobjp = 0; /* it's gone */
    ...
    if (confused) {
        if (Underwater) {
            pline("一点%s在你周围蒸发了。", hliquid("水"));
        }
        else if (Fire_resistance) {
            shieldeff(u.ux, u.uy);
            monstseesu(M_SEEN_FIRE);
            if (!Blind)
                pline("哦，看，你的%s里那团火好漂亮！",
    ...
    if (Underwater) {
        pline_The("你周围的%s剧烈汽化！", hliquid("水"));
    } else {
        if (sblessed) {
            if (!already_known)
                pline("这是一张火焰卷轴！");
    ...
    if (!Is_rogue_level(&u.uz) && has_ceiling(&u.uz)
        ...
        if (u.uswallow) {
            ...
            if (!avoid_ceiling(&u.uz)) {
                pline_The("%s 在你%s轰隆作响！", ceiling(u.ux, u.uy),
                          sblessed ? "周围" : "上方");
            } else {
                char matbuf[BUFSZ];
                ...
                pline("%s的巨石%s%s！",
                      upstart(matbuf), vtense(matbuf, "出现"),
                      sblessed ? "在你周围" : "在你上方");
            }
        }
        gk.known = 1;
...
seffect_magic_mapping(struct obj **sobjp)
{
    ...
    if (is_scroll) {
        if (svl.level.flags.nommap) {
            Your("思想从世俗的关注中释放出来.");
            if (Hallucination)
                pline("当你的内心转向内在, 你忘记了一切.");
            else
                Your("%s 困惑地旋转。", body_part(HEAD));
    ...
    if (svl.level.flags.nommap) {
        Your("%s 旋转着，因为 %s 阻挡了法术！", body_part(HEAD),
             something);
        make_confused(HConfusion + rnd(30), FALSE);
        return;
    }
    pline("一张地图在你的脑海中凝聚成形！");
    ...
    for (j = 0; ; j++) {
        ...
        if (!goodcnt && class != mons[gu.urole.mnum].mlet
            ...
            if (gonecnt)
                ...
                pline("消灭了 %d 个怪物%s。", gonecnt, plur(gonecnt));
                return;
            } else
                pline("那个%s不代表任何怪物.",
                      strlen(buf) == 1 ? "字符" : "回答");
...
do_stinking_cloud(struct obj *sobj, boolean mention_stinking)
{
    ...
    if (getpos(&cc, TRUE, "the desired position") < 0) {
        ...
        if (Hallucination)
            ...
            pline("%s 一股臭鸡蛋味。",
                  sobj->oclass == SCROLL_CLASS ? "卷轴粉碎时伴随着"
                                               : "你闻到");
        return;
    }
