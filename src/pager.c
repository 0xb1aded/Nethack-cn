...
self_lookat(char *outbuf)
{
    ...
    Sprintf(outbuf, "%s%s%s叫做%s",
            /* being blinded may hide invisibility from self */
            (Invis && (senseself() || !Blind)) ? "隐形 " : "", race,
...
look_engrs(boolean nearby)
{
    ...
    for (y = lo_y; y <= hi_y; y++) {
        for (x = lo_x; x <= hi_x; x++) {
            ...
            Sprintf(lookbuf, " (%s", is_headstone ? "坟墓" : "刻字");
