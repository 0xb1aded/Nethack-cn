/* NetHack 5.0	options.c	$NHDT-Date: 1737556914 2025/01/22 06:41:54 $
 * $NHDT-Branch: NetHack-3.7 $:$NHDT-Revision: 1.753 $ */
...
enum optn_result { optn_silenterr = -1, optn_err = 0, optn_ok };
enum requests {
    do_nothing,
    do_init,
    do_set,
...
static NEARDATA const char *OptS_type[OptS_Advanced + 1] = { "一般", "行为",
                                                             "地图", "状态",
                                                             "高级" };
...
static const char none[] = "(无)", randomrole[] = "随机",
                  to_be_done[] = "(待设置)", defopt[] = "默认",
                  defbrief[] = "默认";
...
static const struct paranoia_opts {
    ...
    { PARANOID_DIE, "die", 1, "death", 2,
      "输入yes或y以确认死亡(仅探索模式和调试模式有效)" },
    { PARANOID_BONES, "bones", 1, 0, 0,
      "输入yes或y以在调试模式中保存遗骨层文件" },
    { PARANOID_HIT, "attack", 1, "hit", 1, "输入yes或y以确认攻击和平怪物" },
...
static NEARDATA const char *
    msgwind[][3] = { /* 'msg_window' settings */
                     { "single", "[一次只显示一条信息, 后", "出的先显示]" },
                     { "combination", "[连续按下^P时对前两个使",
                       "用'单', 然后用'full']" },
                     { "full", "[显示所有可用信息, 旧的", "在前, 新的在后]" },
...
static NEARDATA const char *burdentype[] = { "unencumbeded", "burdened",
                                             "stressed",     "strained",
...
static const menu_cmd_t default_menu_cmd_info[] = {
    { "menu_next_page", MENU_NEXT_PAGE, "转到下一页" },
    { "menu_previous_page", MENU_PREVIOUS_PAGE, "转到上一页" },
    { "menu_first_page", MENU_FIRST_PAGE, "转到第一页" },
    { "menu_last_page", MENU_LAST_PAGE, "转到最后一页" },
    { "menu_select_all", MENU_SELECT_ALL, "选中整个菜单中的所有项目" },
    { "menu_invert_all", MENU_INVERT_ALL, "反选所有项目" },
    { "menu_deselect_all", MENU_UNSELECT_ALL,
      "取消选中整个菜单中的所有项目" },
    { "menu_select_page", MENU_SELECT_PAGE, "选中当前页面上的所有项目" },
    { "menu_invert_page", MENU_INVERT_PAGE, "反转当前页面的选中状态" },
    { "menu_deselect_page", MENU_UNSELECT_PAGE,
      "取消选中当前页面上的所有项目" },
    { "menu_search", MENU_SEARCH, "搜索并反选匹配的项目" },
...
ask_do_tutorial(void)
{
    ...
    if (!opt_set_in_config[opt_tutorial]) {
        ...
        Snprintf(buf, sizeof buf,
                 "在%s中加入\"OPTIONS=!tutorial\"以跳过该询问.",
                 (rc && *rc && !norc) ? rc : "你的配置文件");
        do {
            win = create_nhwindow(NHW_MENU);
            start_menu(win, MENU_BEHAVE_STANDARD);
            any = cg.zeroany;
            any.a_char = 'y';
            add_menu(win, &nul_glyphinfo, &any, any.a_char, 0, ATR_NONE,
                     NO_COLOR, "是的, 进行教程", MENU_ITEMFLAGS_NONE);
            any.a_char = 'n';
            add_menu(win, &nul_glyphinfo, &any, any.a_char, 0, ATR_NONE,
                     NO_COLOR, "不, 直接开始游戏.", MENU_ITEMFLAGS_NONE);
...
parseoptions(char *opts, boolean tinitial, boolean tfrom_file)
{
    ...
    for (i = 0; i < OPTCOUNT; ++i) {
        ...
        if (!got_match && allopt[i].name)
            got_match =
                match_optname(opts, allopt[i].name, allopt[i].minmatch, TRUE);
        if (got_match) {
            if (!allopt[i].pfx && optlen < allopt[i].minmatch) {
                config_error_add("Ambiguous option %s, %d characters are "
    ...
    if (!got_match) {
        ...
        for (i = 0; i < OPTCOUNT; ++i) {
            ...
            got_match = match_optname(opts, allopt[i].alias,
                                      (int) strlen(allopt[i].alias), TRUE);
            if (got_match) {
                matchidx = i;
                using_alias = TRUE;
                break;
...
check_misc_menu_command(char *opts, char *op UNUSED)
{
    ...
    for (i = 0; default_menu_cmd_info[i].name; i++) {
        ...
        if (match_optname(opts, name_to_check, (int) strlen(name_to_check),
                          TRUE))
            return i;
    }
    return -1;
...
static int roleopt2opt[4] = { opt_role, opt_race, opt_gender, opt_alignment };
...
getoptstr(int optidx, int ophase)
{
    ...
    if ((roleoptindx >= 0 && roleoptindx < MAX_ROLEOPT && ophase >= 0
         && ophase < num_opt_phases))
...
staticfn int
petname_optfn(int optidx, int req, boolean negated, char *opts, char *op)
{
...
staticfn int
optfn_alignment(int optidx, int req, boolean negated, char *opts, char *op)
{
    if (req == do_init) {
        return optn_ok;
    }
...
staticfn int
optfn_align_message(int optidx, int req, boolean negated, char *opts,
                    char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_altkeyhandling(int optidx UNUSED, int req, boolean negated, char *opts,
                     char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_autounlock(int optidx, int req, boolean negated, char *opts, char *op)
{
    if (req == do_init) {
        ...
        if (!flags.autounlock) {
            ...
            *opts = '\0';
            ...
            if (flags.autounlock & AUTOUNLOCK_FORCE)
                Sprintf(eos(opts), "%s%s", p,
                        unlocktypes[3][0]); /*no more p*/
        }
        return optn_ok;
    }
    if (req == do_handler) {
...
staticfn int
optfn_boulder(int optidx UNUSED, int req, boolean negated UNUSED, char *opts,
              char *op UNUSED)
{
    ...
    if (req == do_set) {
        ...
        if (opts[0] < ' ') {
            ...
            /* for 'initial', update of BOULDER symbol is done in
               ...
               have been processed */
            if (!go.opt_initial) {
                nhsym sym = get_othersym(SYM_BOULDER, Is_rogue_level(&u.uz)
                                                          ? ROGUESET
                                                          : PRIMARYSET);
...
staticfn int
optfn_catname(int optidx, int req, boolean negated, char *opts, char *op)
{
...
#ifdef CRASHREPORT
staticfn int
optfn_crash_email(int optidx UNUSED, int req, boolean negated UNUSED,
                  char *opts, char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_crash_name(int optidx UNUSED, int req, boolean negated UNUSED,
                 char *opts, char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_crash_urlmax(int optidx UNUSED, int req, boolean negated UNUSED,
                   char *opts, char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_cursesgraphics(int optidx, int req, boolean negated, char *opts,
                     char *op UNUSED)
{
#ifdef BACKWARD_COMPAT
...
staticfn int
optfn_DECgraphics(int optidx, int req, boolean negated, char *opts,
                  char *op UNUSED)
{
#ifdef BACKWARD_COMPAT
    boolean badflag = FALSE;
...
staticfn int
optfn_dogname(int optidx, int req, boolean negated, char *opts, char *op)
{
    return petname_optfn(optidx, req, negated, opts, op);
}

staticfn int
optfn_dungeon(int optidx UNUSED, int req, boolean negated UNUSED, char *opts,
              char *op UNUSED)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_effects(int optidx UNUSED, int req, boolean negated UNUSED, char *opts,
              char *op UNUSED)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_font_size_map(int optidx, int req, boolean negated, char *opts,
                    char *op)
{
    /* send them over to the prefix handling for font_ */
    return pfxfn_font(optidx, req, negated, opts, op);
...
staticfn int
optfn_font_size_menu(int optidx, int req, boolean negated, char *opts,
                     char *op)
{
    /* send them over to the prefix handling for font_ */
    return pfxfn_font(optidx, req, negated, opts, op);
...
staticfn int
optfn_font_size_message(int optidx, int req, boolean negated, char *opts,
                        char *op)
{
    /* send them over to the prefix handling for font_ */
    return pfxfn_font(optidx, req, negated, opts, op);
...
staticfn int
optfn_font_size_status(int optidx, int req, boolean negated, char *opts,
                       char *op)
{
    /* send them over to the prefix handling for font_ */
    return pfxfn_font(optidx, req, negated, opts, op);
...
staticfn int
optfn_font_size_text(int optidx, int req, boolean negated, char *opts,
                     char *op)
{
    /* send them over to the prefix handling for font_ */
    return pfxfn_font(optidx, req, negated, opts, op);
...
staticfn int
optfn_fruit(int optidx UNUSED, int req, boolean negated, char *opts, char *op)
{
...
staticfn int
optfn_hilite_status(int optidx UNUSED, int req, boolean negated, char *opts,
                    char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_horsename(int optidx, int req, boolean negated, char *opts, char *op)
{
    return petname_optfn(optidx, req, negated, opts, op);
}

staticfn int
optfn_IBMgraphics(int optidx, int req, boolean negated, char *opts,
                  char *op UNUSED)
{
#ifdef BACKWARD_COMPAT
...
staticfn int
shared_menu_optfn(int optidx UNUSED, int req, boolean negated, char *opts,
                  char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_menu_deselect_all(int optidx, int req, boolean negated, char *opts,
                        char *op)
{
...
staticfn int
optfn_menu_deselect_page(int optidx, int req, boolean negated, char *opts,
                         char *op)
{
    return shared_menu_optfn(optidx, req, negated, opts, op);
}
...
staticfn int
optfn_menu_first_page(int optidx, int req, boolean negated, char *opts,
                      char *op)
{
    return shared_menu_optfn(optidx, req, negated, opts, op);
}
...
staticfn int
optfn_menu_invert_all(int optidx, int req, boolean negated, char *opts,
                      char *op)
{
    return shared_menu_optfn(optidx, req, negated, opts, op);
}
...
staticfn int
optfn_menu_invert_page(int optidx, int req, boolean negated, char *opts,
                       char *op)
{
    return shared_menu_optfn(optidx, req, negated, opts, op);
}
...
staticfn int
optfn_menu_last_page(int optidx, int req, boolean negated, char *opts,
                     char *op)
{
    return shared_menu_optfn(optidx, req, negated, opts, op);
}
...
staticfn int
optfn_menu_next_page(int optidx, int req, boolean negated, char *opts,
                     char *op)
{
    return shared_menu_optfn(optidx, req, negated, opts, op);
}
...
staticfn int
optfn_menu_previous_page(int optidx, int req, boolean negated, char *opts,
                         char *op)
{
    return shared_menu_optfn(optidx, req, negated, opts, op);
}
...
staticfn int
optfn_menu_search(int optidx, int req, boolean negated, char *opts, char *op)
{
    return shared_menu_optfn(optidx, req, negated, opts, op);
}

staticfn int
optfn_menu_select_all(int optidx, int req, boolean negated, char *opts,
                      char *op)
{
    return shared_menu_optfn(optidx, req, negated, opts, op);
}
...
staticfn int
optfn_menu_select_page(int optidx, int req, boolean negated, char *opts,
                       char *op)
{
    return shared_menu_optfn(optidx, req, negated, opts, op);
}
...
staticfn int
optfn_menu_shift_left(int optidx, int req, boolean negated, char *opts,
                      char *op)
{
    return shared_menu_optfn(optidx, req, negated, opts, op);
}
...
staticfn int
optfn_menu_shift_right(int optidx, int req, boolean negated, char *opts,
                       char *op)
{
    return shared_menu_optfn(optidx, req, negated, opts, op);
}
...
staticfn int
optfn_menu_headings(int optidx, int req, boolean negated, char *opts,
                    char *op)
{
    if (req == do_init) {
        return optn_ok;
    ...
    if (req == get_val) {
        ...
        Sprintf(ca_buf, "%s&%s", clr2colorname_ui(iflags.menu_headings.color),
...
staticfn int
optfn_menu_objsyms(int optidx, int req, boolean negated, char *opts, char *op)
{
    if (req == do_init) {
...
staticfn int
optfn_menuinvertmode(int optidx, int req, boolean negated UNUSED, char *opts,
                     char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_monsters(int optidx UNUSED, int req, boolean negated UNUSED, char *opts,
               char *op UNUSED)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_mouse_support(int optidx, int req, boolean negated, char *opts,
                    char *op)
{
    boolean compat;
...
optfn_number_pad(int optidx, int req, boolean negated, char *opts, char *op)
{
    ...
    if (req == get_val || req == get_cnf_val) {
        ...
        int indx = gc.Cmd.num_pad   ? (gc.Cmd.phone_layout
                                           ? (gc.Cmd.pcHack_compat ? 4 : 3)
...
staticfn int
optfn_objects(int optidx UNUSED, int req, boolean negated UNUSED, char *opts,
              char *op UNUSED)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_palette(int optidx UNUSED, int req, boolean negated UNUSED, char *opts,
              char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_paranoid_confirmation(int optidx, int req, boolean opt_negated,
                            char *opts, char *op)
{
    boolean fld_negated;
    int i;
    ...
    if (req == do_set) {
        ...
        if (!strncmpi(opts, "prayconfirm", 4)) {
            if (*op) {
                ...
                config_error_add("deprecated %sprayconfirm option takes no "
                                 "parameters (found '%s')",
                                 opt_negated ? "!" : "", op);
                return optn_silenterr;
            }
            ...
            config_error_add(
                ...
                opt_negated ? "!" : "", allopt[optidx].name,
                opt_negated ? '-' : '+');
            /* keep going */
#endif
    ...
    if (req == get_val || req == get_cnf_val) {
        ...
        for (i = 0; paranoia[i].flagmask != 0; ++i) {
            if ((flags.paranoia_bits & paranoia[i].flagmask) != 0
                ...
                && (paranoia[i].flagmask != PARANOID_BONES || wizard
                    || req == get_cnf_val))
                Snprintf(eos(tmpbuf), sizeof tmpbuf - strlen(tmpbuf), " %s",
                         paranoia[i].argname);
        }
...
staticfn int
optfn_perminv_mode(int optidx, int req, boolean negated, char *opts, char *op)
{
            ...
            for (i = 0; i < SIZE(perminv_modes); ++i) {
                ...
                if (!strncmpi(op, pi0, ln) || !strncmpi(op, pi1, ln)
                    ...
                    if (strstri(pi0, "+grid") && !WINDOWPORT(tty)) {
                        i &= ~InvSparse;
                        config_error_add("%s: unavailable perm_invent mode "
                                         "'%s', using '%s'",
...
staticfn int
optfn_pickup_burden(int optidx, int req, boolean negated UNUSED, char *opts,
                    char *op)
{
    if (req == do_init) {
        return optn_ok;
    ...
    if (req == do_set) {
        ...
        if (op == empty_optstr) {
            ...
            if (flags.menu_style == MENU_TRADITIONAL
                ...
                use_menu = FALSE;
                Sprintf(qbuf, "新%s: [%s am] (%s)", allopt[optidx].name, ocl,
                        *tbuf ? tbuf : "全部");
                abuf[0] = '\0';
                getlin(qbuf, abuf);
            ...
            if (use_menu) {
                ...
                (void) choose_classes_menu("自动拾取什么?", 1, TRUE, ocl,
                                           tbuf);
                op = tbuf;
            }
        }
...
staticfn int
optfn_player_selection(int optidx, int req, boolean negated, char *opts,
                       char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_roguesymset(int optidx, int req, boolean negated UNUSED, char *opts,
                  char *op)
{
    if (req == do_init) {
        return optn_ok;
...
optfn_scores(int optidx, int req, boolean negated, char *opts, char *op)
{
    ...
    if (req == do_set) {
        ...
        while (*op) {
            ...
            case '-':
                if (digit(*(op + 1))) {
                    config_error_add("Values for %s:top and %s:around must "
                                     "not be negative",
                                     allopt[optidx].name,
                                     allopt[optidx].name);
                    return optn_silenterr;
    ...
    if (req == get_val || req == get_cnf_val) {
        *opts = '\0';
        ...
        if (flags.end_around > 0)
            Sprintf(eos(opts), "%s%d around", (flags.end_top > 0) ? "/" : "",
                    flags.end_around);
        if (flags.end_own)
...
staticfn int
optfn_scroll_amount(int optidx, int req, boolean negated, char *opts,
                    char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_scroll_margin(int optidx, int req, boolean negated, char *opts,
                    char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_soundlib(int optidx, int req, boolean negated UNUSED, char *opts,
               char *op)
{
...
staticfn int
optfn_sortdiscoveries(int optidx, int req, boolean negated, char *opts,
                      char *op)
{
    if (req == do_init) {
        flags.discosort = 'o';
...
staticfn int
optfn_sortloot(int optidx, int req, boolean negated UNUSED, char *opts,
               char *op)
{
    int i;
...
staticfn int
optfn_sortvanquished(int optidx, int req, boolean negated, char *opts,
                     char *op)
{
    ...
    if (req == do_handler) {
        ...
        pline("'%s'%s\"%s:%s\".", optname,
              (flags.vanq_sortmode == prev_sortmode) ? "未改变, 仍为"
                                                     : "已更改为",
...
staticfn int
optfn_statushilites(int optidx UNUSED, int req, boolean negated, char *opts,
                    char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_suppress_alert(int optidx, int req, boolean negated, char *opts,
                     char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_symset(int optidx, int req, boolean negated UNUSED, char *opts,
             char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_traps(int optidx UNUSED, int req, boolean negated UNUSED, char *opts,
            char *op UNUSED)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_vary_msgcount(int optidx, int req, boolean negated, char *opts,
                    char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_versinfo(int optidx, int req, boolean negated, char *opts, char *op)
{
    const char *optname = allopt[optidx].name;
    ...
    if (req == do_set) {
        ...
        char vbuf[QBUFSZ];
        boolean g = (vi & VI_NAME) != 0, b = (vi & VI_BRANCH) != 0,
                n = (vi & VI_NUMBER) != 0;
...
#ifdef VIDEOSHADES
staticfn int
optfn_videocolors(int optidx, int req, boolean negated UNUSED, char *opts,
                  char *op UNUSED)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_videoshades(int optidx, int req, boolean negated UNUSED, char *opts,
                  char *op UNUSED)
{
    if (req == do_init) {
        return optn_ok;
...
#ifdef MSDOS
staticfn int
optfn_video_width(int optidx UNUSED, int req, boolean negated, char *opts,
                  char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_video_height(int optidx UNUSED, int req, boolean negated, char *opts,
                   char *op)
{
    if (req == do_init) {
        return optn_ok;
...
#ifdef NO_TERMS
staticfn int
optfn_video(int optidx, int req, boolean negated UNUSED, char *opts,
            char *op UNUSED)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_warnings(int optidx, int req, boolean negated UNUSED, char *opts,
               char *op UNUSED)
{
    int reslt;
...
staticfn int
optfn_whatis_filter(int optidx, int req, boolean negated, char *opts,
                    char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_windowborders(int optidx, int req, boolean negated, char *opts,
                    char *op)
{
    int retval = optn_ok;
    ...
    if (req == get_val) {
        Sprintf(opts, "%s",
                (iflags.wc2_windowborders == 0)   ? "0=关闭"
                : (iflags.wc2_windowborders == 1) ? "1=开启"
                : (iflags.wc2_windowborders == 2) ? "2=自动"
                : (iflags.wc2_windowborders == 3) ? "3=开启, 永久清单除外关闭"
                : (iflags.wc2_windowborders == 4) ? "4=自动, 永久清单除外关闭"
                                                  : defopt);
        return optn_ok;
    }
    if (req == get_cnf_val) {
...
#ifdef WINCHAIN
staticfn int
optfn_windowchain(int optidx, int req, boolean negated UNUSED, char *opts,
                  char *op)
{
    if (req == do_init) {
        return optn_ok;
...
/* Win GUI and curses */
static const char *const wcnames[WC_COUNT] = { "menu", "message", "status",
                                               "text" };
static const char *const wcshortnames[WC_COUNT] = { "mnu", "msg", "sts",
                                                    "txt" };
int wcolors_opt[WC_COUNT];

staticfn int
optfn_windowcolors(int optidx, int req, boolean negated UNUSED, char *opts,
                   char *op)
{
    int wccount;
...
staticfn int
optfn_windowtype(int optidx, int req, boolean negated UNUSED, char *opts,
                 char *op)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
pfxfn_cond_(int optidx UNUSED, int req, boolean negated, char *opts,
            char *op UNUSED)
{
    if (req == do_init) {
...
int
pfxfn_IBM_(int optidx UNUSED, int req, boolean negated UNUSED, char *opts,
           char *op UNUSED)
{
    if (req == do_init) {
        return optn_ok;
...
optfn_boolean(int optidx, int req, boolean negated, char *opts, char *op)
{
    ...
    if (req == do_set) {
        ...
        if (op != empty_optstr) {
            ...
            ln = (int) strlen(op);
            if (!strncmpi(op, "true", ln) || !strncmpi(op, "yes", ln)
                || !strcmpi(op, "on") || (digit(*op) && atoi(op) == 1)) {
                negated = FALSE;
            } else if (!strncmpi(op, "false", ln) || !strncmpi(op, "no", ln)
                       || !strcmpi(op, "off")
                       || (digit(*op) && atoi(op) == 0)) {
                negated = TRUE;
        ...
        *(allopt[optidx].addr) = !negated; /* <==== SET IT HERE */
        ...
        if (give_opt_msg)
            pline("\"%s\"选项已切换为%s.", allopt[optidx].name,
                  !negated ? "开启" : "关闭");
...
handler_align_misc(int optidx)
{
    ...
    any = cg.zeroany;
    any.a_int = ALIGN_TOP;
    add_menu(tmpwin, &nul_glyphinfo, &any, 't', 0, ATR_NONE, clr, "顶部",
             MENU_ITEMFLAGS_NONE);
    any.a_int = ALIGN_BOTTOM;
    add_menu(tmpwin, &nul_glyphinfo, &any, 'b', 0, ATR_NONE, clr, "底部",
             MENU_ITEMFLAGS_NONE);
    any.a_int = ALIGN_LEFT;
    add_menu(tmpwin, &nul_glyphinfo, &any, 'l', 0, ATR_NONE, clr, "左",
             MENU_ITEMFLAGS_NONE);
    any.a_int = ALIGN_RIGHT;
    add_menu(tmpwin, &nul_glyphinfo, &any, 'r', 0, ATR_NONE, clr, "右侧",
             MENU_ITEMFLAGS_NONE);
    Sprintf(abuf, "选择%s窗口相对于地图的放置位置:",
            (optidx == opt_align_message) ? "消息" : "状态");
...
handler_autounlock(int optidx)
{
    ...
    for (i = 0; i < SIZE(unlocktypes); ++i) {
        Sprintf(buf, "%-10.10s%c%.40s", unlocktypes[i][0], sep,
                unlocktypes[i][1]);
    ...
    if ((chngd || flags.verbose) && give_opt_msg) {
        ...
        pline("'%s'%s'%s'.", optname, chngd ? "已更改为" : "未改变, 仍为",
              buf);
    }
    return res;
}
...
handler_disclose(void)
{
    ...
    for (i = 0; i < NUM_DISCLOSURE_OPTIONS; i++) {
        ...
        any.a_int = i + 1;
        add_menu(tmpwin, &nul_glyphinfo, &any, disclosure_options[i], 0,
                 ATR_NONE, clr, buf, MENU_ITEMFLAGS_NONE);
        disc_cat[i] = 0;
    }
    end_menu(tmpwin, "更改哪些揭示选项类别:");
    ...
    for (i = 0; i < NUM_DISCLOSURE_OPTIONS; i++) {
        if (disc_cat[i]) {
            ...
            Sprintf(buf, "针对%s的披露选项:", disclosure_names[i]);
            tmpwin = create_nhwindow(NHW_MENU);
            start_menu(tmpwin, MENU_BEHAVE_STANDARD);
            any = cg.zeroany;
            /* 'y','n',and '+' work as alternate selectors; '-' doesn't */
            any.a_char = DISCLOSE_NO_WITHOUT_PROMPT;
            add_menu(tmpwin, &nul_glyphinfo, &any, 0, any.a_char, ATR_NONE,
                     clr, "绝不披露, 无需提示",
            ...
            if (*disclosure_names[i] == 'v' || *disclosure_names[i] == 'g') {
                ...
                add_menu(tmpwin, &nul_glyphinfo, &any, 0, any.a_char,
                         ATR_NONE, clr, "总是披露, 从菜单中选择排序顺序",
                         (c == any.a_char) ? MENU_ITEMFLAGS_SELECTED
                                           : MENU_ITEMFLAGS_NONE);
            }
...
{
    boolean gotca =
        query_color_attr(&iflags.menu_headings, "如何突出显示菜单标题:");
...
handler_menu_objsyms(void)
{
    ...
    any = cg.zeroany;
    for (i = 0; i < SIZE(objsymvals); ++i) {
        Snprintf(buf, sizeof buf, "%-12.12s%c%.60s", objsymvals[i].nam, sep,
                 objsymvals[i].descr);
        any.a_int = i + 1;
        j = objsymvals[i].num;
        add_menu(tmpwin, &nul_glyphinfo, &any, '0' + i, *buf, ATR_NONE, clr,
                 buf,
                 (j == iflags.menuobjsyms) ? MENU_ITEMFLAGS_SELECTED
                                           : MENU_ITEMFLAGS_NONE);
    }
    end_menu(tmpwin, "将菜单中的物品符号设置成什么?");
...
handler_msg_window(void)
{
    ...
    if (is_tty || is_curses) {
        ...
        boolean chngd;
        int i, n;
        char buf[BUFSZ], c, sep = iflags.menu_tab_sep ? '\t' : ' ',
        ...
        for (i = 0; i < SIZE(menutype); i++) {
            ...
            add_menu(tmpwin, &nul_glyphinfo, &any, *buf, 0, ATR_NONE, clr,
                     buf,
        ...
        if (chngd || flags.verbose) {
            (void) optfn_msg_window(opt_msg_window, get_val, FALSE, buf,
                                    empty_optstr);
        ...
        pline("'%s'选项不支持'%s'.", allopt[opt_msg_window].name,
              windowprocs.name);
    return optn_ok;
}
...
handler_number_pad(void)
{
    ...
    for (i = 0; i < SIZE(npchoices); i++) {
        any.a_int = i + 1;
        add_menu(tmpwin, &nul_glyphinfo, &any, 'a' + i, '0' + i, ATR_NONE,
                 clr, npchoices[i], MENU_ITEMFLAGS_NONE);
    }
    end_menu(tmpwin, "选择数字键盘模式:");
...
handler_paranoid_confirmation(void)
{
    ...
    for (i = 0; paranoia[i].flagmask != 0; ++i) {
        ...
        add_menu(tmpwin, &nul_glyphinfo, &any, *paranoia[i].argname, 0,
                 ATR_NONE, clr, explain,
                 (flags.paranoia_bits & paranoia[i].flagmask)
                     ? MENU_ITEMFLAGS_SELECTED
                     : MENU_ITEMFLAGS_NONE);
    }
    end_menu(tmpwin, "需要额外确认的操作:");
...
handler_perminv_mode(void)
{
    ...
    boolean old_perm_invent = iflags.perm_invent;
    int i, n,
        old_pi = (int) iflags.perminv_mode, new_pi = old_pi,
    ...
    for (i = 0; i < SIZE(perminv_modes); ++i) {
        ...
        any.a_int = i + 1;
        add_menu(tmpwin, &nul_glyphinfo, &any, let, '0' + i, ATR_NONE,
                 NO_COLOR, buf,
                 (i == old_pi) ? MENU_ITEMFLAGS_SELECTED
                               : MENU_ITEMFLAGS_NONE);
    }
...
handler_pickup_burden(void)
{
    ...
    for (i = 0; i < SIZE(burdentype); i++) {
        ...
        any.a_int = i + 1;
        add_menu(tmpwin, &nul_glyphinfo, &any, burden_letters[i], 0, ATR_NONE,
                 clr, burden_name, MENU_ITEMFLAGS_NONE);
    }
    end_menu(tmpwin, "选择负重等级:");
...
handler_runmode(void)
{
    ...
    for (i = 0; i < SIZE(runmodes); i++) {
        ...
        any.a_int = i + 1;
        add_menu(tmpwin, &nul_glyphinfo, &any, *mode_name, 0, ATR_NONE, clr,
                 mode_name, MENU_ITEMFLAGS_NONE);
    }
    end_menu(tmpwin, "选择奔跑/旅行显示模式:");
    if (select_menu(tmpwin, PICK_ONE, &mode_pick) > 0) {
...
{
    int tmp = query_attr("选择宠物突出显示属性", iflags.wc2_petattr);
...
handler_sortloot(void)
{
    ...
    for (i = 0; i < SIZE(sortltype); i++) {
        ...
        add_menu(tmpwin, &nul_glyphinfo, &any, *sortl_name, 0, ATR_NONE, clr,
                 sortl_name,
                 (flags.sortloot == *sortl_name) ? MENU_ITEMFLAGS_SELECTED
                                                 : MENU_ITEMFLAGS_NONE);
    }
    end_menu(tmpwin, "选择搜刮排序类型:");
...
handler_whatis_coord(void)
{
    ...
    add_menu(tmpwin, &nul_glyphinfo, &any, GPCOORDS_COMPASS, 0, ATR_NONE, clr,
             "罗盘('east'或'3s'或'2n,4w')",
             (gpc == GPCOORDS_COMPASS) ? MENU_ITEMFLAGS_SELECTED
                                       : MENU_ITEMFLAGS_NONE);
    any.a_char = GPCOORDS_COMFULL;
    add_menu(tmpwin, &nul_glyphinfo, &any, GPCOORDS_COMFULL, 0, ATR_NONE, clr,
             "全方向罗盘('东'或'3南'或'2北,4西')",
             (gpc == GPCOORDS_COMFULL) ? MENU_ITEMFLAGS_SELECTED
                                       : MENU_ITEMFLAGS_NONE);
    any.a_char = GPCOORDS_MAP;
    add_menu(tmpwin, &nul_glyphinfo, &any, GPCOORDS_MAP, 0, ATR_NONE, clr,
             "地图<x,y>",
             (gpc == GPCOORDS_MAP) ? MENU_ITEMFLAGS_SELECTED
                                   : MENU_ITEMFLAGS_NONE);
    any.a_char = GPCOORDS_SCREEN;
    add_menu(tmpwin, &nul_glyphinfo, &any, GPCOORDS_SCREEN, 0, ATR_NONE, clr,
             "屏幕[行,列]",
             (gpc == GPCOORDS_SCREEN) ? MENU_ITEMFLAGS_SELECTED
                                      : MENU_ITEMFLAGS_NONE);
    any.a_char = GPCOORDS_NONE;
    add_menu(tmpwin, &nul_glyphinfo, &any, GPCOORDS_NONE, 0, ATR_NONE, clr,
             "无(不显示坐标)",
             (gpc == GPCOORDS_NONE) ? MENU_ITEMFLAGS_SELECTED
                                    : MENU_ITEMFLAGS_NONE);
    add_menu_str(tmpwin, "");
    Sprintf(buf, "地图: 左上角: <%d,%d>, 右下角: <%d,%d>%s", 1, 0, COLNO - 1,
            ROWNO - 1, flags.verbose ? ";第0列未使用,超出左边缘" : "");
    add_menu_str(tmpwin, buf);
...
#define COL80ARG ""
#endif
    Sprintf(buf, "屏幕: 左上角: [%02d,%02d], 右下角: [%d,%d]%s", 0 + 2, 1,
            ROWNO - 1 + 2, COLNO - 1, COL80ARG);
#undef COL80ARG
    add_menu_str(tmpwin, buf);
    add_menu_str(tmpwin, "");
    end_menu(tmpwin, "当自动描述地图位置时选择坐标显示方式:");
...
handler_whatis_filter(void)
{
    ...
    add_menu(tmpwin, &nul_glyphinfo, &any, 'n', 0, ATR_NONE, clr, "无过滤",
             (gfilt == GFILTER_NONE) ? MENU_ITEMFLAGS_SELECTED
                                     : MENU_ITEMFLAGS_NONE);
    any.a_char = (GFILTER_VIEW + 1);
    add_menu(tmpwin, &nul_glyphinfo, &any, 'v', 0, ATR_NONE, clr, "仅视野内",
    ...
    add_menu(tmpwin, &nul_glyphinfo, &any, 'a', 0, ATR_NONE, clr,
             ...
             (gfilt == GFILTER_AREA) ? MENU_ITEMFLAGS_SELECTED
                                     : MENU_ITEMFLAGS_NONE);
    end_menu(tmpwin, "选择跳转到下一/上一个地图位置时的地点过滤方式:");
    if ((pick_cnt = select_menu(tmpwin, PICK_ONE, &window_pick)) > 0) {
...
ape_again:
    ...
    if (opt_idx == 3) { /* done */
        ...
        apebuf[0] = apebuf[1] = '\0';
        getlin("添加什么新的自动拾取例外模式?", &apebuf[1]);
        ...
        if (numapes) {
            ape = ga.apelist;
            any = cg.zeroany;
            add_menu_heading(tmpwin, "总是捡起'<'; 从不捡起'>'");
            for (i = 0; i < numapes && ape; i++) {
                ...
                add_menu(tmpwin, &nul_glyphinfo, &any, 0, 0, ATR_NONE, clr,
                         apebuf, MENU_ITEMFLAGS_NONE);
                ape = ape->next;
            }
        }
        Sprintf(apebuf, "%s自动拾取例外",
                (opt_idx == 1) ? "列举" : "移除哪个");
        end_menu(tmpwin, apebuf);
        pick_cnt = select_menu(tmpwin, (opt_idx == 1) ? PICK_NONE : PICK_ANY,
                               &pick_list);
        if (pick_cnt > 0) {
            for (pick_idx = 0; pick_idx < pick_cnt; ++pick_idx)
...
menucolors_again:
    ...
    menucolors_done:
        ...
        mcbuf[0] = '\0';
        getlin("添加什么新的菜单颜色样式?", mcbuf);
        if (*mcbuf == '\033')
            goto menucolors_done;
        if (*mcbuf && test_regex_pattern(mcbuf, "MENUCOLORS regex")
        ...
        while (tmp) {
            ...
            add_menu(tmpwin, &nul_glyphinfo, &any, 0, 0, ATR_NONE, clr, mcbuf,
                     MENU_ITEMFLAGS_NONE);
            tmp = tmp->next;
        }
        Sprintf(mcbuf, "%s菜单颜色", (opt_idx == 1) ? "列举" : "移除哪个");
...
msgtypes_again:
    ...
    if (opt_idx == 3) { /* done */
        ...
        mtbuf[0] = '\0';
        getlin("添加什么新的消息类型样式?", mtbuf);
        if (*mtbuf == '\033')
            return TRUE;
        if (*mtbuf && test_regex_pattern(mtbuf, "MSGTYPE regex")
        ...
        while (tmp) {
            ...
            add_menu(tmpwin, &nul_glyphinfo, &any, 0, 0, ATR_NONE, clr, mtbuf,
                     MENU_ITEMFLAGS_NONE);
            tmp = tmp->next;
        }
        Sprintf(mtbuf, "%s消息类型", (opt_idx == 1) ? "列举" : "移除哪个");
...
handler_windowborders(void)
{
    ...
    static const char *const windowborders_text[] = {
        ...
        "开启, 除非 perm_invent 强制关闭", "自动, 除非 perm_invent 强制关闭"
    };
    ...
    for (i = 0; i < SIZE(windowborders_text); i++) {
        ...
        add_menu(tmpwin, &nul_glyphinfo, &any, 'a' + i, '0' + i, ATR_NONE,
                 clr, mode_name, MENU_ITEMFLAGS_NONE);
    }
    end_menu(tmpwin, "选择窗口边框模式:");
    if (select_menu(tmpwin, PICK_ONE, &mode_pick) > 0) {
...
{
#ifdef MICRO
    pline("\"%s\"只能在%s里设置.", optname, get_configfile());
#else
    pline("%s只能在NETHACKOPTIONS或%s里设置.", optname, get_configfile());
#endif
}
...
initoptions_init(void)
{
    ...
    if ((opts = nh_getenv("TERM"))
        ...
        && !strncmpi(opts, "vt", 2) && AS && AE && strchr(AS, '\016')
        && strchr(AE, '\017')) {
        if (!gs.symset[PRIMARYSET].explicitly)
...
feature_alert_opts(char *op, const char *optn)
{
    ...
    if (!go.opt_initial) {
        Sprintf(buf, "%lu.%lu.%lu", FEATURE_NOTICE_VER_MAJ,
                FEATURE_NOTICE_VER_MIN, FEATURE_NOTICE_VER_PATCH);
        pline("已禁用NetHack %s及之前版本的功能变更提醒.", buf);
    }
    return 1;
}
...
parsebindings(char *bindings)
{
    ...
    static const char *const mousebtn_names[NUM_MOUSE_BUTTONS] = { "mouse1",
                                                                   "mouse2" };
...
static const struct {
    ...
    xint8 msgtyp;
    const char *descr;
} msgtype_names[] = { { "show", MSGTYP_NORMAL, "正常显示消息" },
                      { "hide", MSGTYP_NOSHOW, "隐藏消息" },
                      { "noshow", MSGTYP_NOSHOW, NULL },
                      { "stop", MSGTYP_STOP, "多条消息后提示more" },
                      { "more", MSGTYP_STOP, NULL },
                      { "norep", MSGTYP_NOREP, "不要重复消息" } };
...
staticfn boolean
parse_role_opt(int optidx, boolean negated, const char *fullname, char *opts,
               char **opp)
{
        ...
        while (*op) {
            ...
            if (!first) {
                if ((val_negated ^ prev_negated)
                    ...
                    config_error_add("Multiple role values only allowed when "
                                     "list is negated");
                    return FALSE;
                }
            }
                          ...
                          * commands to control or meta characters */
{
    ...
    static const struct menuscrollinfo scroll_keys[] = {
        { MENU_FIRST_PAGE, 1 }, { MENU_PREVIOUS_PAGE, 1 },
        { MENU_NEXT_PAGE, 2 },  { MENU_LAST_PAGE, 2 },
        { MENU_SHIFT_LEFT, 4 }, { MENU_SHIFT_RIGHT, 8 },
    };
    int i;
...
DISABLE_WARNING_FORMAT_NONLITERAL /* RESTORE is after show_menucontrols() */
    ...
    staticfn int
    optfn_o_autopickup_exceptions(int optidx UNUSED, int req,
                                  boolean negated UNUSED, char *opts,
                                  char *op UNUSED)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_o_bind_keys(int optidx UNUSED, int req, boolean negated UNUSED,
                  char *opts, char *op UNUSED)
{
    if (req == do_init) {
        return optn_ok;
...
staticfn int
optfn_o_autocomplete(int optidx UNUSED, int req, boolean negated UNUSED,
                     char *opts, char *op UNUSED)
{
    if (req == do_init) {
        return optn_ok;
...
longest_option_name(int startpass, int endpass)
{
    ...
    for (pass = 0; pass < 2; pass++)
        for (i = 0; (name = allopt[i].name) != 0; i++) {
            if (pass == 0 && (allopt[i].opttyp != BoolOpt || !allopt[i].addr))
                continue;
...
redo_opt_help:
    ...
    add_menu(tmpwin, &nul_glyphinfo, &any, '?', 0, ATR_NONE, NO_COLOR,
             gs.simple_options_help ? "隐藏帮助" : "显示帮助",
             MENU_ITEMFLAGS_NONE);
    ...
    for (section = OptS_General; section < OptS_Advanced; section++) {
        ...
        for (i = 0; (name = allopt[i].name) != 0; i++) {
            ...
            case OthrOpt:
                ...
                reslt = optn_err;
                if (allopt[k].optfn)
                    reslt = (*allopt[k].optfn)(allopt[k].idx, get_val, FALSE,
                                               buf2, empty_optstr);
                Sprintf(buf, fmtstr, name,
                        ((reslt == optn_ok && buf2[0]) ? (const char *) buf2
                                                       : "未知"));
                break;
            default:
                Sprintf(buf, "错误");
    ...
    if (pick_cnt > 0) {
        ...
        if (k == -2) {
            ...
            if (allopt[k].has_handler && allopt[k].optfn) {
                ...
                Sprintf(buf, "将%s设置为什么?", allopt[k].name);
                getlin(buf, abuf);
                if (abuf[0] != '\033') { /* ESC */
...
term_for_boolean(int idx, boolean *b)
{
    ...
    static const char *const booleanterms[2][num_terms + 4] = {
        { "false", "off", "disabled", "excluded from build", "关", "禁",
          "关闭", "禁用" },
...
rerun:
    ...
    indexoffset = 1;
    any = cg.zeroany;
    add_menu_heading(tmpwin, "布尔值(按下对应键以开关):");
    any.a_int = 0;
    ...
    for (pass = 0; pass <= 1; pass++)
        for (i = 0; (name = allopt[i].name) != 0; i++)
            if (allopt[i].opttyp == BoolOpt && (bool_p = allopt[i].addr) != 0
                ...
                Sprintf(buf, fmtstr_doset, indent, name,
                        term_for_boolean(i, bool_p));
                if (pass == 0)
                    enhance_menu_text(buf, sizeof buf, pass, bool_p,
                                      &allopt[i]);
                add_menu(tmpwin, &nul_glyphinfo, &any, 0, 0, ATR_NONE, clr,
                         buf, MENU_ITEMFLAGS_SKIPINVERT);
            }

    add_menu_str(tmpwin, "");
    add_menu_heading(tmpwin, "复杂值(按下对应键以赋新值):");
    ...
    add_menu_str(tmpwin, "");
    add_menu_heading(tmpwin, "变量游乐场位置:");
    ...
    if ((pick_cnt = select_menu(tmpwin, PICK_ANY, &pick_list)) > 0) {
        ...
        for (pick_idx = 0; pick_idx < pick_cnt; ++pick_idx) {
            ...
            if (allopt[opt_indx].opttyp == BoolOpt) {
                ...
                if (allopt[k].has_handler && allopt[k].optfn) {
                    reslt =
                        (*allopt[k].optfn)(allopt[k].idx, do_handler, FALSE,
                                           empty_optstr, empty_optstr);
                    /* if player eventually saves options, include this one */
                    if (reslt == optn_ok)
                        opt_set_in_config[k] = TRUE;
                    ...
                    Sprintf(buf, "将%s设置为什么?", allopt[opt_indx].name);
                    abuf[0] = '\0';
                    getlin(buf, abuf);
                    if (abuf[0] == '\033')
                        continue;
                                    ...
                                    * or zero if option cannot be changed */
{
    ...
    if (i >= 0 && i < OPTCOUNT && allopt[i].name && allopt[i].optfn) {
        ...
        if (allopt[i].optfn)
            reslt = (*allopt[i].optfn)(allopt[i].idx, get_val, FALSE, buf2,
                                       empty_optstr);
    ...
    if (dolist) { /* key bindings help: '?i' */
        ...
        for (i = 0; default_menu_cmd_info[i].desc; i++) {
            ...
            if ((ch == MENU_SHIFT_RIGHT || ch == MENU_SHIFT_LEFT)
                && !has_menu_shift)
                continue;
            Sprintf(buf, fmt, visctrl(get_menu_cmd_key(ch)),
        ...
        putstr(win, 0, "");
        Sprintf(buf, mc_altfmt, "", "整个", "当前");
        putstr(win, 0, buf);
        Sprintf(buf, mc_altfmt, "", "菜单", "页");
        putstr(win, 0, buf);
        Sprintf(buf, mc_fmt, "选择",
                visctrl(get_menu_cmd_key(MENU_SELECT_ALL)),
                visctrl(get_menu_cmd_key(MENU_SELECT_PAGE)));
        putstr(win, 0, buf);
        Sprintf(buf, mc_fmt, "反转",
                visctrl(get_menu_cmd_key(MENU_INVERT_ALL)),
                visctrl(get_menu_cmd_key(MENU_INVERT_PAGE)));
        putstr(win, 0, buf);
        Sprintf(buf, mc_fmt, "取消",
                visctrl(get_menu_cmd_key(MENU_UNSELECT_ALL)),
                visctrl(get_menu_cmd_key(MENU_UNSELECT_PAGE)));
        putstr(win, 0, buf);
        putstr(win, 0, "");
        Sprintf(buf, mc_fmt, "进入",
                visctrl(get_menu_cmd_key(MENU_NEXT_PAGE)), "下一页");
        putstr(win, 0, buf);
        Sprintf(buf, mc_fmt, "",
                visctrl(get_menu_cmd_key(MENU_PREVIOUS_PAGE)), "上一页");
        putstr(win, 0, buf);
        Sprintf(buf, mc_fmt, "", visctrl(get_menu_cmd_key(MENU_FIRST_PAGE)),
                "第一页");
        putstr(win, 0, buf);
        Sprintf(buf, mc_fmt, "", visctrl(get_menu_cmd_key(MENU_LAST_PAGE)),
                "最后一页");
        putstr(win, 0, buf);
        if (has_menu_shift) {
            Sprintf(buf, mc_fmt, "全景视图",
                    visctrl(get_menu_cmd_key(MENU_SHIFT_RIGHT)),
                    "右(仅限perm_invent)");
            putstr(win, 0, buf);
            Sprintf(buf, mc_fmt, "",
                    visctrl(get_menu_cmd_key(MENU_SHIFT_LEFT)), "右");
            putstr(win, 0, buf);
        }
        putstr(win, 0, "");
        Sprintf(buf, mc_fmt, "搜索", visctrl(get_menu_cmd_key(MENU_SEARCH)),
                "提取目标字符串, 并将所有匹配项反转");
        putstr(win, 0, buf);
        /* separator before hardcoded */
        putstr(win, 0, "");
        fmt = "%9s  %-8s %s";
        arg = "其他"; /* prefix for first hardcoded[] entry, then reset */
    }
...
handle_add_list_remove(const char *optname, int numtotal)
{
    ...
    for (i = 0; i < SIZE(action_titles); i++) {
        ...
        add_menu(tmpwin, &nul_glyphinfo, &any, action_titles[i].letr, 0,
                 ATR_NONE, clr, tmpbuf,
...
toggle_bool_option(const char *p)
{
    ...
    for (i = 0; i < OPTCOUNT; i++)
        if (!strncmpi(allopt[i].name, p, strlen(p))
            ...
            && allopt[i].setwhere == set_in_game && allopt[i].addr != 0) {
            char buf[BUFSZ];
...
option_help(void)
{
    ...
    Snprintf(buf, sizeof buf, "在%s中设置OPTIONS=<options>",
             get_configfile());
        ...
        *newop++ = '\0';
        ...
        for (j = 0; j < WC_COUNT; ++j) {
            if (!strcmpi(wn, wcnames[j]) || !strcmpi(wn, wcshortnames[j])) {
                ...
                if (wcolors_opt[j] != 0) {
                    config_error_add("windowcolors for %s windows specified "
                                     "multiple times",
                                     wcnames[j]);
                }
                wcolors_opt[j]++;
...
staticfn void
enhance_menu_text(char *buf, size_t sz, int whichpass UNUSED, boolean *bool_p,
