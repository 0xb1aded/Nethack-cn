    ...
    char *opts, char *op)
{
    ...
    if (req == get_val || req == get_cnf_val) {
        ...
        Sprintf(opts, "%s",
                (which == ALIGN_TOP) ? "顶部"
                : (which == ALIGN_LEFT) ? "左侧"
                  : (which == ALIGN_BOTTOM) ? "底部"
                    : (which == ALIGN_RIGHT) ? "右侧"
                      : defopt);
        return optn_ok;
    }
    if (req == do_handler) {
    ...
    char *opts, char *op)
{
    ...
    if (req == do_init) {
        ...
        if (iflags.perminv_mode != InvOptNone && !iflags.perm_invent
            ...
            Strcat(opts, (((iflags.perminv_mode & InvSparse) != 0) ? " (关闭)"
                          : " ('perm_invent' 已关闭)"));
        }
    ...
    char *opts, char *op)
{
    ...
    if (req == get_val) {
        Sprintf(opts, "%s", (gp.preferred_pet == 'c') ? "猫"
                           : (gp.preferred_pet == 'd') ? "狗"
                             : (gp.preferred_pet == 'h') ? "马"
                               : (gp.preferred_pet == 'n') ? "无"
                                 : "随机");
        return optn_ok;
    }
    if (req == get_cnf_val) {
    ...
    char *opts, char *op)
{
    ...
    if (req == get_val || req == get_cnf_val) {
        Sprintf(opts, "%s",
                iflags.wc_player_selection ? "提示" : "对话框");
        return optn_ok;
    }
    return optn_ok;
}
    ...
    char *opts, char *op)
{
    ...
    if (req == get_val || req == get_cnf_val) {
        Strcpy(opts, wizard ? "调试" : discover ? "探索" : "正常");
        return optn_ok;
    }
    return optn_ok;
}
    ...
    char *opts, char *op)
{
    ...
    if (req == get_val || req == get_cnf_val) {
        *opts = '\0';
        if (flags.end_top > 0)
            Sprintf(opts, "%d 名", flags.end_top);
        if (flags.end_around > 0)
            Sprintf(eos(opts), "%s%d 周围",
                    (flags.end_top > 0) ? "/" : "", flags.end_around);
        if (flags.end_own)
            Sprintf(eos(opts), "%s自己的",
    ...
    char *opts, char *op)
{
    ...
    if (req == get_val) {
        ...
        if (!iflags.hilite_delta)
            Strcpy(opts, "0（关闭：不突出显示状态字段）");
        else
            Sprintf(opts, "%ld (开启：高亮状态持续%ld回合)",
    ...
    char *opts, char *op)
{
    ...
    if (req == do_set) {
        ...
        pline("'%s' %s %u.", optname,
              (flags.versinfo == vi) ? "没有改变，仍然是" : "改变为",
              flags.versinfo);
    } else if (req == get_val) {
        char vbuf[QBUFSZ];
    ...
    char *opts, char *op)
{
    ...
    if (req == get_val || req == get_cnf_val) {
        Sprintf(opts, "%s",
                (iflags.getpos_coords == GPCOORDS_MAP) ? "地图"
                : (iflags.getpos_coords == GPCOORDS_COMPASS) ? "指南针"
                : (iflags.getpos_coords == GPCOORDS_COMFULL) ? "完整指南针"
                : (iflags.getpos_coords == GPCOORDS_SCREEN) ? "屏幕"
                : "无");
        return optn_ok;
    }
    if (req == do_handler) {
    ...
    char *opts, char *op)
{
    ...
    if (req == get_val || req == get_cnf_val) {
        Sprintf(opts, "%s",
                (iflags.getloc_filter == GFILTER_VIEW) ? "视野"
                : (iflags.getloc_filter == GFILTER_AREA) ? "区域"
                : "无");
        return optn_ok;
    }
    if (req == do_handler) {
...
handler_menustyle(void)
{
    ...
    if (chngd || flags.verbose)
        pline("'menustyle' %s \"%s\".", chngd ? "改为" : "仍为",
...
handler_align_misc(int optidx)
{
    ...
    add_menu(tmpwin, &nul_glyphinfo, &any, 'r', 0, ATR_NONE, clr, "右侧",
             MENU_ITEMFLAGS_NONE);
    Sprintf(abuf, "选择 %s 窗口相对于地图的放置位置:",
...
handler_menu_objsyms(void)
{
    ...
    end_menu(tmpwin, "菜单中的物品符号要设置成什么？");
...
complain_about_duplicate(int optidx)
{
    ...
    buf[0] = '\0';
    if (using_alias)
        Sprintf(buf, " (通过别名: %s)", allopt[optidx].alias);
...
#else
    pline("%s 只能通过 NETHACKOPTIONS 或 %s 设置。", optname,
          get_configfile());
#endif
}
    ...
    if (pick_cnt > 0) {
        ...
        if (k == -2) {
            ...
            if (allopt[k].has_handler && allopt[k].optfn) {
                ...
                Sprintf(buf, "%s设置为什么?", allopt[k].name);
                getlin(buf, abuf);
                if (abuf[0] != '\033') { /* ESC */
        ...
        for (pick_idx = 0; pick_idx < pick_cnt; ++pick_idx) {
            ...
            if (allopt[opt_indx].opttyp == BoolOpt) {
                ...
                if (allopt[k].has_handler && allopt[k].optfn) {
                    ...
                    Sprintf(buf, "%s设置为什么?", allopt[opt_indx].name);
                    abuf[0] = '\0';
                    getlin(buf, abuf);
                    if (abuf[0] == '\033')
                        continue;
