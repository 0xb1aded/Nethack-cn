...
static NEARDATA const char *burdentype[] = {
    "unencumbeded",   "burdened", "stressed",
    ...
    if (req == do_set) {
        ...
        if (op == empty_optstr) {
            ...
            if (flags.menu_style == MENU_TRADITIONAL
                ...
                use_menu = FALSE;
                Sprintf(qbuf, "新%s: [%s am] (%s)", allopt[optidx].name,
                        ocl, *tbuf ? tbuf : "全部");
                abuf[0] = '\0';
                getlin(qbuf, abuf);
            ...
            if (use_menu) {
                ...
                (void) choose_classes_menu("自动拾取什么?",
                                           1, TRUE, ocl, tbuf);
                op = tbuf;
            }
        }
...
{
    int tmp
        = query_attr("选择宠物突出显示属性", iflags.wc2_petattr);
...
handler_whatis_filter(void)
{
    ...
    add_menu(tmpwin, &nul_glyphinfo, &any, 'a',
             0, ATR_NONE, clr, "在同一区域",
             (gfilt == GFILTER_AREA)
                ? MENU_ITEMFLAGS_SELECTED : MENU_ITEMFLAGS_NONE);
    end_menu(tmpwin,
      "选择跳转到下一/上一个地图位置时的地点过滤方式:");
    if ((pick_cnt = select_menu(tmpwin, PICK_ONE, &window_pick)) > 0) {
...
#else
    pline("%s只能在NETHACKOPTIONS或%s里设置.", optname,
          get_configfile());
#endif
}
...
option_help(void)
{
    ...
    Snprintf(buf, sizeof buf,
             "在%s中设置OPTIONS=<options>", get_configfile());
