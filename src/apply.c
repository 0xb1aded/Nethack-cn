...
{
    const char thump[] = "砰! 你的一击被%s无伤弹开.";
...
use_cream_pie(struct obj *obj)
{
    ...
    if (Hallucination)
        ...
        You("把你的%s浸入到%s%s.", body_part(FACE),
              several ? "一个" : "",
...
use_royal_jelly(struct obj **optr)
{
    ...
    eobj = getobj("涂抹蜂王浆", jelly_ok, GETOBJ_PROMPT);
    if (!eobj) {
        if (splitit) {
            (void) unsplitobj(obj);
...
doapply(void)
{
    ...
    obj = getobj("使用", apply_ok, GETOBJ_NOFLAGS);
    if (!obj)
        return ECMD_CANCEL;
    ...
    case EUCALYPTUS_LEAF:
        ...
        if (obj->blessed) {
            ...
            if (!rn2(49)) {
                if (!Blind) {
                    pline("%s %s.", Yobjnam2(obj, "发出"), hcolor("褐色"));
                    set_bknown(obj, 1);
                }
                unbless(obj);
            }
...
flip_through_book(struct obj *obj)
{
    ...
    if (obj->otyp == SPE_BOOK_OF_THE_DEAD) {
        if (!Deaf) {
            ...
            You_see("书页微微发%s光.", hcolor(NH_RED));
        } else {
            You_feel("书页在颤抖.");
        }
    } else if (Blind) {
