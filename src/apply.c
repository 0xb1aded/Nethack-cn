...
#define SEENMON (MONSEEN_NORMAL | MONSEEN_SEEINVIS | MONSEEN_INFRAVIS)
    ...
    if (mtmp->msleeping) {
        ...
        if (vis) /* (redundant) */
            pline("%s在黑暗中。",
                  monverbself(mtmp, Monnam(mtmp), "是",
                              "太远而看不见"));
...
jump(int magic) /* 0=Physical, otherwise skill level */
{
    ...
    /* if steed is immobile, can't do physical jump but can do spell one */
        ...
        You("无法挣脱%s的控制!", mon_nam(u.ustuck));
        return ECMD_OK;
...
use_tinning_kit(struct obj *obj)
{
    ...
    if (touch_petrifies(mptr) && !Stone_resistance && !uarmg) {
        ...
        if (poly_when_stoned(gy.youmonst.data)) {
            You("罐装%s没有戴手套.", corpse_name);
            kbuf[0] = '\0';
        } else {
            pline("罐装%s不戴手套是个致命的错误...",
                  corpse_name);
    ...
    if ((can = mksobj(TIN, FALSE, FALSE)) != 0) {
        static const char you_buy_it[] = "你罐装了它, 你就要买它!";
        ...
        case OBJ_MINVENT:
            if (cansee_spot && !silent && !suppress_see) {
                ...
                if (canseemon(figurine->ocarry)
                    ...
                    Sprintf(carriedby, "%s包袱", s_suffix(a_monnam(mon)));
                else if (is_pool(mon->mx, mon->my))
                    Strcpy(carriedby, "空水");
                else
                    Strcpy(carriedby, "稀薄的空气");
                You_see("%s %s 从 %s%s!", monnambuf,
                        locomotion(mtmp->data, "掉出"), carriedby,
                        and_vanish);
            }
            break;
#if 0
...
use_grease(struct obj *obj)
{
    ...
    if (Glib) {
        pline("%s 从你的 %s 上滑落。", Tobjnam(obj, "滑落"),
    ...
    if (obj->spe > 0) {
        ...
        if ((obj->cursed || Fumbling) && !rn2(2)) {
            ...
            pline("%s从你的%s上滑落。", Tobjnam(obj, "滑落"),
                  fingers_or_gloves(FALSE));
            dropx(obj);
            return ECMD_TIME;
        }
...
use_stone(struct obj *tstone)
{
    ...
    Sprintf(stonebuf, "在石头上摩擦%s", plur(tstone->quan));
        ...
        if (y_n(buf) == 'y') {
            if (chance) {
                ...
                case BEAR_TRAP: /* drop it without arming it */
                    reset_trapset();
                    You("扔下%s!", the(trapname(ttyp, FALSE)));
                    dropx(otmp);
                    return;
                }
            }
...
set_trap(void)
{
    ...
    if (ttmp) {
        ...
        if (!gt.trapinfo.force_bungle)
            You("完成安装%s.", the(trapname(ttyp, FALSE)));
...
use_whip(struct obj *obj)
{
    ...
    if (u.uswallow) {
        ...
        Sprintf(buf, "%s自己死于%s牛鞭", uhim(), uhis());
...
use_cream_pie(struct obj *obj)
{
    ...
    if (Hallucination)
        ...
        You("把你的%s浸入%s%s中。", body_part(FACE),
              several ? "其中一个" : "",
...
use_royal_jelly(struct obj **optr)
{
    ...
    if (obj->cursed) {
        if (eobj->timed || eobj->corpsenm != oldcorpsenm)
            pline("这%s微弱地%s。", xname(eobj), otense(eobj, "颤抖"));
        else
            pline("%s", nothing_seems_to_happen);
        kill_egg(eobj);
        goto useup_jelly;
...
use_grapple(struct obj *obj)
{
    ...
    if (typ != P_NONE && P_SKILL(typ) >= P_SKILLED) {
        ...
        any.a_int++;
        Sprintf(buf, "这个%s", surface(cc.x, cc.y));
...
doapply(void)
{
    ...
    case LENSES:
        if (obj == ublindf) {
            ...
            You("已经%s了。",
                (ublindf->otyp == TOWEL) ? "被毛巾遮住"
                : (ublindf->otyp == BLINDFOLD) ? "戴着蒙眼布"
                  : "戴着眼镜");
        }
        break;
    case CREAM_PIE:
    ...
    case EUCALYPTUS_LEAF:
        ...
        if (obj->blessed) {
            ...
            if (!rn2(49)) {
                if (!Blind) {
                    pline("%s %s.", Yobjnam2(obj, "发出"), hcolor("棕色光"));
                    set_bknown(obj, 1);
                }
                unbless(obj);
            }
...
flip_through_book(struct obj *obj)
{
    ...
    if (obj->otyp == SPE_BOOK_OF_THE_DEAD) {
        if (!Deaf) {
            ...
            You_see("书页微微发光%s。", hcolor(NH_RED));
        } else {
            You_feel("书页在颤抖。");
        }
    } else if (Blind) {
...
flip_coin(struct obj *obj)
{
    ...
    You("抛掷 %s。", an(singular(obj, xname)));
    if (Underwater) {
        pline("它翻滚着漂走了。");
        lose_coin = TRUE;
    } else if (Glib || Fumbling
               || (ACURR(A_DEX) < 10 && !rn2(ACURR(A_DEX)))) {
        pline("它从你的%s之间滑落。", fingers_or_gloves(FALSE));
        lose_coin = TRUE;
    }
